...
public class StringUtility 
{
	...
	static public boolean isEmpty(String strInput)
	{
		...
		if(strInput == null)
			return true;
		return strInput.length() == key_0 ? true : false;
		*/
	...
	static protected byte StringToByte(String strInput)
	{
		...
		//Log.i("APP", String.format("byteArry[key_0] = 0x%X\n", byteArry[key_0]));
	...
	static public String ByteArrayToStringindex(byte[] arryByte, int nDataLength,int begin)
	{
		String strOut = new String();
		for(int i = 0; i < nDataLength; i++)
			strOut += String.format("%02X ", arryByte[begin]);
		return strOut;
	}
}
